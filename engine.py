...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    i = 0
    write_interval = 100
    num_frame = cfg.DATALOADER.TRAIN.NUM_FRAMES
    for samples, targets in metric_logger.log_every(data_loader, print_freq, writer, args, epoch, header):
        ...
        samples_1 = utils.NestedTensor(samples.tensors[:, 0:num_frame, :, :, :], samples.mask[:, 0:num_frame, :, :])
        samples_2 = utils.NestedTensor(samples.tensors[:, num_frame:, :, :, :], samples.mask[:, num_frame:, :, :])
        targets_1 = copy.deepcopy(targets)
        targets_2 = copy.deepcopy(targets)
        for tg in targets_1:
            for key in tg.keys():
                if tg[key].shape[0] == cfg.DATALOADER.TRAIN.NUM_FRAMES * 2:
                    tg[key] = tg[key][0:num_frame]
        for tg in targets_2:
            for key in tg.keys():
                if tg[key].shape[0] == cfg.DATALOADER.TRAIN.NUM_FRAMES * 2:
                    tg[key] = tg[key][num_frame:]

        outputs_1, model_need_to_write = model(samples_1, captions, targets_1)     #模型得到输出？
        loss_dict_1, loss_need_to_write = criterion(outputs_1, targets_1, args, cfg)     #计算loss函数项

        weight_dict = criterion.weight_dict     #loss的权重系数
        losses_1 = sum(loss_dict_1[k] * weight_dict[k] for k in loss_dict_1.keys() if k in weight_dict)       #total loss

        # reduce losses over all GPUs for logging purposes
        loss_dict_reduced_1 = utils.reduce_dict(loss_dict_1)        #对所有进程中的loss值sum/avg，然后替换每个进程中的loss，使每个进程中的loss统一
        loss_dict_reduced_unscaled_1 = {f'{k}_unscaled_1': v        #不乘权重系数的loss
                                      for k, v in loss_dict_reduced_1.items()}
        loss_dict_reduced_scaled_1 = {k: v * weight_dict[k]       #乘权重系数的loss
                                    for k, v in loss_dict_reduced_1.items() if k in weight_dict}
        losses_reduced_scaled_1 = sum(loss_dict_reduced_scaled_1.values())  #对所有loss项求和

        loss_value_1 = losses_reduced_scaled_1.item()   #精度更高

        if not math.isfinite(loss_value_1):   #判断是否有限
            print("Loss is {}, stopping training".format(loss_value_1))
            print(loss_dict_reduced_1)
            sys.exit(1)

        optimizer.zero_grad()   #清空梯度
        losses_1.backward()   #反向传播梯度

        if max_norm > 0:    #gradient clipping
            grad_total_norm_1 = torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm)
        else:
            grad_total_norm_1 = utils.get_total_grad_norm(model.parameters(), max_norm)
        optimizer.step()

        metric_logger.update(loss=loss_value_1, **loss_dict_reduced_scaled_1, **loss_dict_reduced_unscaled_1)
        ...
        metric_logger.update(grad_norm=grad_total_norm_1)

        prompt_token = outputs_1['prompt_token']

        outputs_2, model_need_to_write = model(samples_2, captions, targets_2, video_prompt=prompt_token)     #模型得到输出？
        loss_dict_2, loss_need_to_write = criterion(outputs_2, targets_2, args, cfg)     #计算loss函数项

        weight_dict = criterion.weight_dict     #loss的权重系数
        losses_2 = sum(loss_dict_2[k] * weight_dict[k] for k in loss_dict_2.keys() if k in weight_dict)       #total loss

        # reduce losses over all GPUs for logging purposes
        loss_dict_reduced_2 = utils.reduce_dict(loss_dict_2)        #对所有进程中的loss值sum/avg，然后替换每个进程中的loss，使每个进程中的loss统一
        loss_dict_reduced_unscaled_2 = {f'{k}_unscaled_2': v        #不乘权重系数的loss
                                      for k, v in loss_dict_reduced_2.items()}
        loss_dict_reduced_scaled_2 = {k: v * weight_dict[k]       #乘权重系数的loss
                                    for k, v in loss_dict_reduced_2.items() if k in weight_dict}
        losses_reduced_scaled_2 = sum(loss_dict_reduced_scaled_2.values())  #对所有loss项求和

        loss_value_2 = losses_reduced_scaled_2.item()   #精度更高

        if not math.isfinite(loss_value_2):   #判断是否有限
            print("Loss is {}, stopping training".format(loss_value_2))
            print(loss_dict_reduced_2)
            sys.exit(1)

        optimizer.zero_grad()   #清空梯度
        losses_2.backward()   #反向传播梯度

        if max_norm > 0:    #gradient clipping
            grad_total_norm_2 = torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm)
        else:
            grad_total_norm_2 = utils.get_total_grad_norm(model.parameters(), max_norm)
        optimizer.step()

        metric_logger.update(loss=loss_value_2, **loss_dict_reduced_scaled_2, **loss_dict_reduced_unscaled_2)
        metric_logger.update(lr=optimizer.param_groups[0]["lr"])
        metric_logger.update(lr_encoder=optimizer.param_groups[1]["lr"])
        metric_logger.update(lr_decoder=optimizer.param_groups[2]["lr"])
        metric_logger.update(lr_pretrained_module=optimizer.param_groups[3]["lr"])
        metric_logger.update(grad_norm=grad_total_norm_2)
