...
def main(args, cfg):
    ...
    model, criterion, postprocessor = build_model_clip(args, cfg, batch_size=cfg.DATALOADER.TRAIN.BATCH_SIZE)
    # model, criterion, postprocessor = build_model_clip(args, cfg) #构建模型结构     返回模型、loss函数、后处理器
    model.to(device)
    n_parameters = sum(p.numel() for n, p in model.named_parameters() if p.requires_grad)  # numel()返回数组中元素的个数，用来得到网络中参数的总数目
    ...
    print(f'{n_parameters:,} total parameters.')
    print(f'{n_parameters/(1024*1024):.2f}M total parameters.')'''
    ...
    if args.resume:     #从checkpoint恢复/继续
        if args.resume.startswith('https'):     #字符串是否以https开头，是的话得下载checkpoint
            ...
            # checkpoint = pre_trained_model_to_finetune(checkpoint, args)
